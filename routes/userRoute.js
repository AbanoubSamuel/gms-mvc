...
router.post('/register', async function (req, res)
{
  ...
  {
    return res.status(400).send({ message: "Please make sure to submit all the required data" });
  }
  ...
  User.create({ ...body, password: hashedPassword }, function (err, data)
  {
    ...
    {
      // console.log(err);
      res.status(405).send({ message: "User is already registered, Please login instead !" });
    }
    else
    {
...
router.post("/login", async (req, res) =>
{
  ...
  if (!body.email || !body.password)
  {
    return res.status(403).send({ message: "Please make sure to submit all the required data" });
  } else
  {
    const user = await User.findOne({ email: body.email }).populate({ path: 'clientIds' }).populate({ path: 'trainerId' });
    if (user)
    {
...
router.patch('/update_user', middle, function (req, res)
{
  ...
  User.findOneAndUpdate({ email: req.body.email }, { ...req.body }, function (err, data)
  {
    try
    {
      if (err)
...
router.get('/attendce', middle, async (req, res) =>
{
  ...
  if (foundCode.length != 0 && foundCode[foundCode.length - 1].date == new Date().toDateString())
  {
...
router.post("/gateway", middle, function (req, res)
{
  ...
  }).populate({ path: 'clientIds' }).populate({ path: 'trainerId' });
});
